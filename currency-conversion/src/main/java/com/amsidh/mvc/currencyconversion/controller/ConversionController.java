...
public class ConversionController {
  ...
  @GetMapping(value = {"/from/{currencyFrom}/to/{currencyTo}/quantity/{quantity}"}, produces = {
  ...
  public CurrencyConversionResponse convertCurrency(
    ...
    String currencyExchangeUrlFullPath =
        currencyExchangeUrl + "/currency-exchange/" + currencyFrom + "/to/" + currencyTo;
    log.info("Calling CurrencyExchange service with url " + currencyExchangeUrlFullPath);
    Exchange exchange = restTemplate.getForEntity(currencyExchangeUrlFullPath, Exchange.class)
        .getBody();
