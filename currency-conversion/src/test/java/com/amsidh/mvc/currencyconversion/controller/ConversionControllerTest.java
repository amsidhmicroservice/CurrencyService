...
class ConversionControllerTest {
  ...
  void testConvertCurrency() throws Exception {
    ...
    Mockito.when(restTemplate.getForEntity("http://localhost:8181/currency-exchange/USD/to/INR",
            Exchange.class))
    ...
    Mockito.when(instanceInformationService.retrieveInstanceInfo())
        .thenReturn(conversionEnvironmentInfo);

    log.info("Created the request");
    //Created the request
    MockHttpServletRequestBuilder mockHttpServletRequestBuilder = MockMvcRequestBuilders.get(
        "/currency-conversion/from/{currencyFrom}/to/{currencyTo}/quantity/{quantity}",
        "USD", "INR", 100);
    log.info("Call the API and get the response");
    //Call the API and get the response
    MockHttpServletResponse response = mockMvc.perform(mockHttpServletRequestBuilder)
