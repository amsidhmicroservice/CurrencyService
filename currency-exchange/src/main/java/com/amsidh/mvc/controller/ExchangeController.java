...
public class ExchangeController {
    ...
    public CurrencyExchange getCurrencyExchange(@PathVariable("currencyFrom") String currencyFrom, @PathVariable("currencyTo") String currencyTo) {
        ...
        log.info("Json In Single Line {}", new ObjectMapper().writerWithDefaultPrettyPrinter().writeValueAsString(currencyExchangeEntity));
        log.info("Json In Single Line {}", new ObjectMapper().writeValueAsString(currencyExchangeEntity));

        String json= "{\n" +
                "  \"id\" : 10001,\n" +
                "  \"currencyFrom\" : \"USD\",\n" +
                "  \"currencyTo\" : \"INR\",\n" +
                "  \"conversionMultiple\" : 65.00000000000000000000000000000000000000000000000000,\n" +
                "  \"exchangeEnvironmentInfo\" : \"1.1.6-RELEASE : local\"\n" +
                "}";
        log.info("Multiline Json: {}"+ json);
    ...
    public ResponseEntity<String> throwException(@PathVariable(name = "records") Integer records) {
        log.info("Initializing throwException…");
